...
// objetoIds = IDs de modelo (objetos_modelo). Se crean filas en tabla objetos con stats desde el modelo.
app.post('/api/user/dar-objetos', async (req, res) => {
  ...
  try {
    ...
    const missing = modeloIds.filter((id) => statsByModelo[id] === undefined);
    if (missing.length > 0) {
      return res.status(400).json({
        success: false,
        message: 'Estos IDs de modelo no existen en la BD estática (objetos_modelo): ' + missing.join(', '),
